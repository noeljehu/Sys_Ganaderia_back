...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                ...
                .sessionManagement(session -> session.sessionCreationPolicy(SessionCreationPolicy.STATELESS)) // Nueva forma de gestionar sesión
                .authorizeHttpRequests(auth -> auth
                        .requestMatchers("/auth/login", "*").permitAll()  // Permitir acceso público al login
                        .anyRequest().permitAll()                        // Permitir acceso público a todas las rutas
                );
